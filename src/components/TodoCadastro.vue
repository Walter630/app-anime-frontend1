<template>
  <v-container
    ...
    fluid
    style="
      background: url('/imgs/Atras.jpg') no-repeat center center / cover;
      min-height: 100vh;
    "
  >
    <v-card
      class="pa-6 rounded-xl ma-5 cadastro"
      width="400"
      elevation="4"
      style="box-shadow: 10px 10px 20px #bec8d2, -10px -10px 20px #ffffff"
    >
      <v-img src="/LogoYokuso.png" pa-2 height="80"></v-img>
      ...
      <form @submit.prevent="addUsuario()">
        ...
        <small
          v-if="mensagem && mensagemTipo === 'error' && !email.includes('@gmail.com')"
          class="text-error"
        >
          {{ mensagem }}
        </small>
        ...
        <small
          v-if="mensagem && mensagemTipo === 'error' && senha.length < 8"
          class="text-error"
        >
          {{ mensagem }}
        </small>
...
.cadastro {
  ...
  backdrop-filter: blur(4px); /* opcional: adiciona um efeito de desfoque no fundo */
}
</style>
